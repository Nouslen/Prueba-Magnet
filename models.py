...
class DogHouse(object):
    ...
    def get_data(self, token: str):
        ...
        """
        # CREA TODOS LOS PERROS
        dogs_list = get("http://dogs.magnet.cl/api/v1/dogs/", token)
        for perro in dogs_list['results']:
            dog = Dog(perro['id'], perro['name'], perro['breed'])
            self.dogs.append(dog)

        #CREA TODAS LAS RAZAS
        breed_list = get("http://dogs.magnet.cl/api/v1/breeds/", token)
        for raza in breed_list['results']:
            breed = Breed(raza['id'], raza['name'])
            self.breeds.append(breed)
            for perroCreado in self.dogs: # RECORRE LA LISTA DE PERROS YA CREADOS Y COMPARA SU ID PARA AGREGAR PERROS (add_dog)
                if perroCreado.id == breed.id:
                    breed.add_dog(perroCreado)
    ...
    def get_common_breed(self) -> Breed:
        ...
        common_breed = 0 
        common_name = ""
        for raza in self.breeds:
            if raza.dogs_count() > common_breed:
                common_breed = raza.dogs_count()
                common_name = raza
        return common_name
    ...
    def get_common_dog_name(self) -> str:
       ...
       raise NotImplementedError
        """
        set_names = set(self.dogs)
        common_dog = {}
        count_names = 0
        
        for dog in set_names:
            for perro in self.dogs:
                if perro.name == dog.name:
                    count_names += 1
            common_dog[dog.name] = count_names
            count_names = 0
        return max(common_dog, key=common_dog.get)

        #for key, value in common_dog.items():
         #   print(key, ' : ', value)
